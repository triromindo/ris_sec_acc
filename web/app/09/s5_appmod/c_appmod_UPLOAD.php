...
if ($varToStep == 3) {
    ...
    if ($isRun) {
        ...
        foreach ($tmpIDSel as $key => $valIDSel) {
            if (array_key_exists($valIDSel, $_SESSION['tmp']['table_appmod'])) {
                $data = $_SESSION['tmp']['table_appmod'][$valIDSel];

                $tmpQueryString = sprintf("
                    INSERT INTO `11sys_rec__appmod_id` 
                        (`rec_id`, `rec_status`, `last_action`, 
                        `last_req_user`, `last_req_timestamp`, `last_app_user`, `last_app_timestamp`, 
                        `app_id`, `appmod_id`, `appmod_name`, `appmod_index`, 
                        `appmod_navname`, `appmod_group`, `appmod_type`, `appmod_ver`,
                    ...
                    VALUES 
                        ...
                        '%s', '%s', '%s', '%s', 
                        '%s', '%s'); ",
                        $data['app'], $data['id'], $data['name'], $data['index'],
                        $data['navname'], $data['group'], $data['type'], $data['ver'],
                        $data['note'], $data['acc']);
    ...
    if ($isRun) {
        $PAGE['step'] = 3;
        unset($_SESSION['tmp']['table_appmod']);
    } else {
        $PAGE['step'] = 2;
    ...
    if ($isRun) {
        ...
        if ($tmpResult = $varMySqli->query($tmpQueryString)) {
            while ($row = $tmpResult->fetch_assoc()) {
                $tmpDatabaseKey[$row['appmod_id']] = $row['appmod_id'];
            }
        } else {
            $data['status'] = false;
        ...
        if (($tmpFileHandle = fopen($_FILES["fileToUpload"]["tmp_name"], "r")) !== FALSE) {
            ...
            foreach ($PAGE['table'] as $key => $value) {
                $_SESSION['tmp']['table_appmod'][$value['id']] = $value;

                //  Cek apakah key ada dalam data. Bila ada diberi tanda
                if (array_key_exists($value['id'], $tmpDatabaseKey)) {
                    $_SESSION['tmp']['table_appmod'][$value['id']]['double'] = true;
                } else {
                    $_SESSION['tmp']['table_appmod'][$value['id']]['double'] = false;
                }
            }
